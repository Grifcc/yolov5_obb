...
from utils.poly_iou import poly_iou
...
from utils.general import (LOGGER, check_dataset, check_img_size, check_requirements, check_yaml,
                           colorstr, increment_path,save_one_txt, print_args,scale_rbox, non_max_suppression_obb)
...
def process_batch(detections, labels, iouv):
    ...
    Arguments:
        ...
        labels (Array[M, 5]), class, x, y, w, h, θ
    Returns:
        correct (Array[N, 10]), for 10 IoU levels
    """
    correct = torch.zeros(detections.shape[0], iouv.shape[0], dtype=torch.bool, device=iouv.device)
    iou = poly_iou(labels[:, 1:], detections[:, :5])
...
def run(data,
    ...
    if not training:
        ...
        dataloader = create_dataloader(data[task],imgsz, batch_size, stride, names_list,data['rbox'],single_cls, pad=pad, rect=pt,
                                       workers=workers, prefix=colorstr(f'{task}: '))[0]

    seen = 0
    confusion_matrix = ConfusionMatrix(nc=nc)
    names = {k: v for k, v in enumerate(model.names if hasattr(model, 'names') else model.module.names)}
    s = ('%20s' + '%11s' * 6) % ('Class', 'Images', 'Labels', 'P', 'R', 'OBBmAP@.5', '  OBBmAP@.5:.95')
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        for si, pred in enumerate(out): # pred (tensor): (n, [xylsθ, conf, cls])
            ...
            if single_cls:
                pred[:, 6] = 0
            prbox= pred.clone()
            prbox=scale_rbox(im[si].shape[1:], prbox, shape, shapes[si][1])
            # Evaluate
            if nl:
                trbox=scale_rbox(im[si].shape[1:], labels[:,1:], shape, shapes[si][1],use_pi=True)
                trbox=torch.cat((labels[:,0:1],trbox), dim=1)
                correct = process_batch(prbox, trbox, iouv)
                if plots:
                    confusion_matrix.process_batch(prbox, trbox)
            else:
                correct = torch.zeros(pred.shape[0], niou, dtype=torch.bool)
            stats.append((correct.cpu(), prbox[:, -2].cpu(), prbox[:, -1].cpu(), tcls))  # (correct, conf, pcls, tcls)
            # Save/log
            if save_txt: # just save hbb pred results!
                save_one_txt(prbox, save_conf, file=save_dir / 'labels' / (path.stem + '.txt'),names=names_list) 
                # LOGGER.info('The horizontal prediction results has been saved in txt, which format is [cls cx cy w h /conf/]')
            callbacks.run('on_val_image_end', prbox, prbox, path, names, im[si])
