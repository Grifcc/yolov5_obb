...
def scale_rbox(img1_shape, labels, img0_shape, ratio_pad=None,use_pi=False):
    # ratio_pad: [(h_raw, w_raw), (hw_ratios, wh_paddings)]
    # Rescale rbox (xywhθ) from img1_shape to img0_shape
    polys=labels.clone()
    if ratio_pad is None:  # calculate from img0_shape
        gain = min(img1_shape[0] / img0_shape[0], img1_shape[1] / img0_shape[1])  # gain  = resized / raw
        pad = (img1_shape[1] - img0_shape[1] * gain) / 2, (img1_shape[0] - img0_shape[0] * gain) / 2  # wh padding
    ...
    polys[:, 0] -= pad[0]  # x padding
    polys[:, 1] -= pad[1]  # y padding
    polys[:, :4] /= gain # Rescale poly shape to img0_shape
    #clip_polys(polys, img0_shape)
    if use_pi:
        polys[:,4]=((polys[:,4]/pi*180+90).int())%180  #θ∈[0,180) int
    return polys

def save_one_txt(pred, save_conf, file,names):
    # Save one txt result
    if isinstance(pred, torch.Tensor):
        pred=pred.tolist()
    for *xywha, conf, cls in pred:
        cls=names[int(cls)]
        if save_conf:
            line =('%7.3f ' *4 +'%3d '+ "%-12s "+ '%.3f')%(*xywha,cls, conf)# label format
        else:
            line =('%7.3f ' *4 +'%3d '+ "%-12s ")%(*xywha,cls)
...
def non_max_suppression_obb(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Detections matrix nx7 (xyls, θ, conf, cls) θ ∈ int[0, 180)
        if multi_label:
