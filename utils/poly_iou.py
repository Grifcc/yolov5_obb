...
'''
import torch
from utils.poly_op.cuda_ext import sort_v
import numpy as np
import cv2
...
def rbox2poly(obboxes):
    ...
    Args:
        rboxes (array/tensor): (B,N, [cx cy l s θ]) θ∈[0,180)

    Returns:
        polys (array/tensor): (B,N, [x1 y1 x2 y2 x3 y3 x4 y4]) 
    """

    center, w, h, theta = obboxes[..., :2], obboxes[..., 2:3], obboxes[..., 3:4], (obboxes[..., 4:5] -90.0)/180 *np.pi
    ...
    return torch.cat(
        (point1, point2, point3, point4), dim=-1).reshape(*order, 4,2)
...
def oriented_box_intersection_2d(corners1:torch.Tensor, corners2:torch.Tensor):
    ...
    Args:
        corners1 (torch.Tensor): (B, N, 5)
        corners2 (torch.Tensor): (B, N, 5)

    Returns:
        area: (B, N), area of intersection
        selected: (B, N, 9, 2), vertices of polygon with zero padding 
    """
    corners1=rbox2poly(corners1)
    corners2=rbox2poly(corners2)
...
def poly_iou(box1:torch.Tensor,box2:torch.Tensor):
    box1=box1[:,None,:].repeat(1,box2.shape[0],1)
    box2=box2[None,:,:].repeat(box1.shape[0],1,1)
    tensor1=box1.view(box1.shape[0],box1.shape[1],-1)
    tensor2=box2.view(box2.shape[0],box2.shape[1],-1)
    area, _ = oriented_box_intersection_2d(tensor1, tensor2)
    s1=tensor1[...,2]*tensor1[...,3]
    s2=tensor2[...,2]*tensor2[...,3]
    iou = area / (s1+s2-area)
    return iou


def draw(img,boxes,color,thickness):
    for i in range(4):
        cv2.line(img, tuple(boxes[i].numpy().astype(int)), tuple(boxes[(i+1)%4].numpy().astype(int)), color, thickness)
        
if __name__ == "__main__":
    
    ppoly=np.load("pred_polyn.npy")
    tpoly=np.load("tpoly_withcls.npy")
    ppoly=torch.from_numpy(ppoly)
    tpoly=torch.from_numpy(tpoly)
    boxes1=ppoly[...,:8].cuda()
    boxes2=tpoly[...,1:].cuda()
