...
def poly2rbox(polys, num_cls_thata=180, radius=6.0, use_pi=False, use_gaussian=False):
    ...
    for poly in polys:
        ...
        if not use_pi: # 采用angle弧度制 θ ∈ [0， 180)
            rboxes.append([x, y, w, h, int(angle)])
        else: # 采用pi制
...
def make_gaussian(angle,num_cls_thata, radius):
    csl_labels=[]
    for i in angle[:,-1]:
        csl_label = gaussian_label_cpu(label=i, num_class=num_cls_thata, u=0, sig=radius)
        csl_labels.append(csl_label)
    rboxes=angle.copy()
    rboxes[:,-1]=(rboxes[:,-1]-90) / 180 * pi
    return rboxes,csl_labels
...
def rbox2poly(obboxes):
    ...
    if isinstance(obboxes, torch.Tensor):
        center, w, h, theta = obboxes[:, :2], obboxes[:, 2:3], obboxes[:, 3:4], (obboxes[:, 4:5] -90.0)/180 *pi
...
def rbox2poly_withcls(obboxes):
    ...
    """
    if isinstance(obboxes, torch.Tensor):
        cls,center, w, h, theta =obboxes[:,0],obboxes[:, 1:3], obboxes[:, 3:4], obboxes[:, 4:5], obboxes[:, 5:6]
        Cos, Sin = torch.cos(theta), torch.sin(theta)
        ...
        order = obboxes.shape[:-1]
        return torch.cat(
            (cls.reshape(*order, 1),point1, point2, point3, point4), dim=-1).reshape(*order, 9)
    else:
        cls,center, w, h, theta = np.split(obboxes, (1,3,4,5), axis=-1)
        Cos, Sin = np.cos(theta), np.sin(theta)
        ...
        order = obboxes.shape[:-1]
        return np.concatenate(
            [cls.reshape(*order,1),point1, point2, point3, point4], axis=-1).reshape(*order,9)

# def convert_gaussian(polys, num_cls_thata=180, radius=6.0, use_pi=False, use_gaussian=False):
#     for i in polys:
#         csl_label = gaussian_label_cpu(label=angle, num_class=num_cls_thata, u=0, sig=radius)
#         csl_labels.append(csl_label)
