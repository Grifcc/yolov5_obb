...
from utils.rboxs_utils import poly_filter, poly2rbox,make_gaussian
...
def create_dataloader(path, imgsz, batch_size, stride, names, rbox=False,single_cls=False, hyp=None, augment=False, cache=False, pad=0.0,
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, cls_names, img_size=640, batch_size=16, rbox=False,augment=False, hyp=None, rect=False, image_weights=False,
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        with Pool(NUM_THREADS) as pool:
            pbar = tqdm(pool.imap(verify_image_label, zip(self.img_files, self.label_files, repeat(prefix), repeat(self.cls_names),repeat(self.rbox))),
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if labels.size:  
                ...
                if self.rbox:
                    labels[:, 3] = img_label[:, 3] * ratio[0]
                    labels[:, 4] = img_label[:, 4] * ratio[1]
                    labels[:, 1] = img_label[:, 1] * ratio[0] + pad[0]
                    labels[:, 2] = img_label[:, 2] * ratio[1] + pad[1]
        ...
        if self.augment:
            ...
            if 1:
                ...
                if nl:
                    # labels[:, 2] = 1 - labels[:, 2]
                    if self.rbox:
                        labels[:, 2]=img_h - labels[:, 2] - 1
                        labels[:, 4]=180-labels[:, 4]
                    else:
        ...
        # *[clsid poly] to *[clsid cx cy l s theta gaussian_θ_labels] θ∈[-pi/2, pi/2) non-normalized
            if self.rbox:
                # rboxes = labels[:,1:]
                rboxes,csl_labels=make_gaussian(angle=labels[:,1:], 
    ...
    for i, index in enumerate(indices):
        ...
        if labels.size:
            # labels[:, 1:] = xywhn2xyxy(labels[:, 1:], w, h, padw, padh)  # normalized xywh to pixel xyxy format\
                #here scale
            if self.rbox:
                labels[:, 1] = img_label[:,1] + padw
                labels[:, 2] = img_label[:,2] + padh
            else:
...
def verify_image_label(args):
    ...
    im_file, lb_file,prefix, cls_name_list,rbox= args
            ...
            with open(lb_file) as f:
                ...
                for label in labels:
                    if label[-1] == "2": # diffcult
                        continue
                    # print(cls_name_list)
                    cls_id = cls_name_list.index(label[-2])
                    if rbox:
                        l_.append(np.concatenate((cls_id, label[:5]), axis=None))
                    else:
                        l_.append(np.concatenate((cls_id, label[:8]), axis=None))
                l = np.array(l_, dtype=np.float32)
            nl = len(l)
            if nl:
                assert len(label) == 10 if not rbox else 7, f'Yolov5-OBB labels require 10  or 6 columns, which same as DOTA Dataset, {len(label)} columns detected'
                ...
                l = np.zeros((0, 9 if not rbox else 6 ), dtype=np.float32)
        else:
            nm = 1  # label missing
            # l = np.zeros((0, 5), dtype=np.float32)
            l = np.zeros((0, 9 if not rbox else 6 ), dtype=np.float32)
