...
from utils.general import (LOGGER, check_file, check_img_size, check_imshow, check_requirements, colorstr,
                           increment_path, scale_rbox, non_max_suppression_obb, print_args, strip_optimizer, save_one_txt)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                prbox = scale_rbox(im.shape[2:], prbox, im0.shape)# (n, [xylsθ conf cls])
                ...
                if save_txt:  # Write to file
                    save_one_txt(det,save_conf, file=txt_path+".txt",names=names)
                for *rbox, conf, cls in det:
                    if save_img or save_crop or view_img:  # Add poly to image
                        c = int(cls)  # integer class
                        label = None if hide_labels else (names[c] if hide_conf else f'{names[c]} {conf:.2f}')
                        # annotator.box_label(xyxy, label, color=colors(c, True))
                        poly=rbox2poly(torch.stack(rbox).reshape(1,-1)).squeeze().cpu()
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default='../val/images', help='file/dir/URL/glob, 0 for webcam')
